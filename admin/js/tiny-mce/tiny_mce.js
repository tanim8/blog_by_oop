(function(d) {
    ...
    var b = {majorVersion: "3", minorVersion: "4.6", releaseDate: "2011-09-29", _init: function() {
...
(function(a) {
    a.VK = {DELETE: 46, BACKSPACE: 8}
})(tinymce);
(function(i) {
    var g = i.VK, h = g.BACKSPACE, f = g.DELETE;
    function b(j) {
        var l = j.dom, k = j.selection;
        j.onKeyDown.add(function(n, r) {
            var m, s, p, q, o;
            o = r.keyCode == f;
            if (o || r.keyCode == h) {
                r.preventDefault();
                m = k.getRng();
                s = l.getParent(m.startContainer, l.isBlock);
                if (o) {
                    s = l.getNext(s, l.isBlock)
                }
                if (s) {
                    p = s.firstChild;
                    while (p.nodeType == 3 && p.nodeValue.length == 0) {
                        p = p.nextSibling
                    }
                    if (p && p.nodeName === "SPAN") {
                        q = p.cloneNode(false)
                    }
                }
                n.getDoc().execCommand(o ? "ForwardDelete" : "Delete", false, null);
                s = l.getParent(m.startContainer, l.isBlock);
                i.each(l.select("span.Apple-style-span,font.Apple-style-span", s), function(t) {
                    var u = k.getBookmark();
                    if (q) {
                        l.replace(q.cloneNode(false), t, true)
                    } else {
                        l.remove(t, true)
                    }
                    k.moveToBookmark(u)
    ...
    function c(j) {
        j.onKeyUp.add(function(k, m) {
            var l = m.keyCode;
            if (l == f || l == h) {
                if (k.dom.isEmpty(k.getBody())) {
                    k.setContent("", {format: "raw"});
                    k.nodeChanged();
                    return
                }
            }
    ...
    function a(j) {
        j.dom.bind(j.getDoc(), "focusin", function() {
            j.selection.setRng(j.selection.getRng())
        })
    }
    function e(j) {
        if (!Range.prototype.getClientRects) {
            j.onMouseDown.add(function(l, m) {
                if (m.target.nodeName === "HTML") {
                    var k = l.getBody();
                    k.blur();
                    setTimeout(function() {
                        k.focus()
    ...
    function d(j) {
        j.onClick.add(function(k, l) {
            l = l.target;
            if (/^(IMG|HR)$/.test(l.nodeName)) {
                k.selection.getSel().setBaseAndExtent(l, 0, l, 1)
            }
            if (l.nodeName == "A" && k.dom.hasClass(l, "mceItemAnchor")) {
                k.selection.select(l)
            }
            k.nodeChanged()
        })
    }
    i.create("tinymce.util.Quirks", {Quirks: function(j) {
            if (i.isWebKit) {
                b(j);
                c(j);
                a(j);
...
(function(m) {
    ...
    m.create("tinymce.Editor", {Editor: function(r, q) {
                ...
                i(g(v.plugins), function(t) {
                    if (t && !c.urls[t]) {
                        if (t.charAt(0) == "-") {
                            ...
                            i(s, function(z) {
                                var y = {prefix: "plugins/", resource: z, suffix: "/editor_plugin" + m.suffix + ".js"};
                            ...
                            c.load(t, {prefix: "plugins/", resource: t, suffix: "/editor_plugin" + m.suffix + ".js"})
                        }
                    }
                });
            ...
            if (I.render_ui) {
                ...
                q = H.theme.renderUI({targetNode: D, width: E, height: A, deltaWidth: I.delta_width, deltaHeight: I.delta_height});
                H.editorContainer = q.editorContainer
            }
            if (document.domain && location.hostname != document.domain) {
                m.relaxedDomain = document.domain
            }
            n.setStyles(q.sizeContainer || q.editorContainer, {width: E, height: A});
            if (I.content_css) {
            ...
            H.iframeHTML += '</head><body id="' + y + '" class="mceContentBody ' + C + '"><br></body></html>';
            ...
            r = n.add(q.iframeContainer, "iframe", {id: H.id + "_ifr", src: F || 'javascript:""', frameBorder: "0", allowTransparency: "true", title: I.aria_label, style: {width: "100%", height: A, display: "block"}});
            ...
            var q = this, v = q.settings, x = n.get(q.id), y = q.getDoc(), u, p;
            if (!b || !m.relaxedDomain) {
                y.open();
                y.write(q.iframeHTML);
                y.close();
                if (m.relaxedDomain) {
            ...
            q.dom = new m.dom.DOMUtils(q.getDoc(), {keep_values: true, url_converter: q.convertURL, url_converter_scope: q, hex_colors: v.force_hex_style_colors, class_filter: v.class_filter, update_styles: 1, fix_ie_paragraphs: 1, schema: q.schema});
            q.parser = new m.html.DomParser(v, q.schema);
            if (!q.settings.allow_html_in_named_anchor) {
                q.parser.addAttributeFilter("name", function(s, t) {
                    var A = s.length, C, z, B, D;
                    while (A--) {
                        D = s[A];
                        if (D.name === "a" && D.firstChild) {
                            B = D.parent;
                            C = D.lastChild;
                            do {
                                z = C.prev;
            ...
            q.parser.addAttributeFilter("src,href,style", function(s, t) {
                var z = s.length, B, D = q.dom, C, A;
                while (z--) {
                    B = s[z];
                    C = B.attr(t);
                    A = "data-mce-" + t;
                    if (!B.attributes.map[A]) {
                        if (t === "style") {
                            B.attr(A, D.serializeStyle(D.parseStyle(C), B.name))
                        } else {
                            B.attr(A, q.convertURL(C, t, B.name))
                        }
                    }
                }
            });
            q.parser.addNodeFilter("script", function(s, t) {
                var z = s.length;
                while (z--) {
                    s[z].attr("type", "mce-text/javascript")
                }
            });
            q.parser.addNodeFilter("#cdata", function(s, t) {
                ...
                while (z--) {
                    ...
                    A.type = 8;
                    A.name = "#comment";
                    A.value = "[CDATA[" + A.value + "]]"
                }
            });
            q.parser.addNodeFilter("p,h1,h2,h3,h4,h5,h6,div", function(t, z) {
                var A = t.length, B, s = q.schema.getNonEmptyElements();
                while (A--) {
                    B = t[A];
                    if (B.isEmpty(s)) {
                        B.empty().append(new m.html.Node("br", 1)).shortEnded = true
                    }
                }
            });
            ...
            i("p h1 h2 h3 h4 h5 h6 div address pre div code dt dd samp".split(/\s/), function(s) {
                q.formatter.register(s, {block: s, remove: "all"})
            });
            ...
            q.forceBlocks = new m.ForceBlocks(q, {forced_root_block: v.forced_root_block});
            ...
            if (v.handle_node_change_callback) {
                q.onNodeChange.add(function(t, s, z) {
                    q.execCallback("handle_node_change_callback", q.id, z, -1, -1, true, q.selection.isCollapsed())
                })
            }
            if (v.save_callback) {
                q.onSaveContent.add(function(s, z) {
                    var t = q.execCallback("save_callback", q.id, z.content, q.getBody());
                    if (t) {
                        z.content = t
                    }
                })
            ...
            if (v.protect) {
                q.onBeforeSetContent.add(function(s, t) {
                    if (v.protect) {
                        i(v.protect, function(z) {
                            t.content = t.content.replace(z, function(A) {
            ...
            if (v.verify_css_classes) {
                q.serializer.attribValueFilter = function(B, z) {
                    var A, t;
                    if (B == "class") {
                        if (!q.classesRE) {
                            t = q.dom.getClasses();
                            if (t.length > 0) {
                                ...
                                q.classesRE = new RegExp("(" + A + ")", "gi")
                            }
                        }
                        return !q.classesRE || /(\bmceItem\w+\b|\bmceTemp\w+\b)/g.test(z) || q.classesRE.test(z) ? z : ""
                    }
                    return z
                }
            }
            ...
            if (v.handle_event_callback) {
                q.onEvent.add(function(s, t, z) {
                    if (q.execCallback("handle_event_callback", t, s, z) === false) {
                        j.cancel(t)
                    }
                })
            }
            ...
            if (a) {
                function r(s, t) {
                    i(s.dom.select("a"), function(A) {
                        var z = A.parentNode;
                        if (s.dom.isBlock(z) && z.lastChild === A) {
                            s.dom.add(z, "br", {"data-mce-bogus": 1})
                        }
                    })
                }
            ...
            q.startContent = q.getContent({format: "raw"});
            q.undoManager.add();
            q.initialized = true;
            q.onInit.dispatch(q);
    ...
    a.Formatter = function(V) {
        ...
        function A(Y, ah, ab) {
            ...
            if (!q.isCollapsed() || !aj.inline || c.select("td.mceSelected,th.mceSelected").length) {
                ...
                ak(q.getRng(B));
                q.moveToBookmark(ag);
                if (aj.inline && j(Y, ah, q.getStart())) {
                    aa(q.getRng(true))
                }
                V.nodeChanged()
            } else {
